...
const createClassJoinUserSchema = Joi.object({
  class_id: commonValidations.class_id,
  status: commonValidations.status
});

const updateClassJoinUserSchema = Joi.object({
  id: Joi.number()
    .integer()
    .positive()
    .required()
    .messages({
      'number.base': 'Class Join Map User ID must be a number',
      'number.integer': 'Class Join Map User ID must be an integer',
      'number.positive': 'Class Join Map User ID must be a positive number',
      'any.required': 'Class Join Map User ID is required'
    }),
...
const getClassJoinUserByIdSchema = Joi.object({
  id: Joi.number()
    .integer()
    .positive()
    .required()
...
const getAllClassJoinUserSchema = Joi.object({
  page: Joi.number()
    .integer()
    .min(1)
    .default(1)
