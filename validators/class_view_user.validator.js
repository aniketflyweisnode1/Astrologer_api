...
const createClassViewUserSchema = Joi.object({
  class_id: commonValidations.class_id,
  status: commonValidations.status
});

const updateClassViewUserSchema = Joi.object({
  id: Joi.number()
    .integer()
    .positive()
    .required()
    .messages({
      'number.base': 'Class View Map User ID must be a number',
      'number.integer': 'Class View Map User ID must be an integer',
      'number.positive': 'Class View Map User ID must be a positive number',
      'any.required': 'Class View Map User ID is required'
    }),
...
const getClassViewUserByIdSchema = Joi.object({
  id: Joi.number()
    .integer()
    .positive()
    .required()
...
const getAllClassViewUserSchema = Joi.object({
  page: Joi.number()
    .integer()
    .min(1)
    .default(1)
